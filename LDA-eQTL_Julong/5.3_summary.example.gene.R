...
option <- "DiagLDA2"
outdir <- paste(option, "/examples/", sep="")
if (!file.exists(outdir))  dir.create(outdir, showWarnings=F, recursive=T)


########################################
### 1. significant pairs of SNP-Gene ###
########################################

rm(list=ls())

dataset <- read.table("dataset_contrast.txt", header=F)[-10,]
...
###    
   cat(paste(cell, "_", lda, "_", treat, sep=""), "\n")
    
### LDA interaction results
   fn <- paste0(option, "/4.2_lm.results/2_", cell, "_lda", lda, "_trt", treat, "_stratified.FDR.txt")
   res1 <- fread(fn)

###response
###
   res1 <- res1%>%filter(interaction_ANOVA.qvalue_stratified<0.1)

   res1 <- res1%>%mutate(ENSG2=gsub("\\..*", "", ENSG),
                         MCls=cell, LDA=lda, treat=treat)%>%
     left_join(gene_key, by=c("ENSG2"="ensgene"))%>%
     dplyr::select(MCls, LDA, treat, ENSG, ENSG2, symbol, varID,
                   interaction_ANOVA.pvalue)
    
   res1 <- res1%>%arrange(interaction_ANOVA.pvalue) 

   res1
})

opfn <- paste(option, "/summary.examples/signif_pairs-SNP-eGene.txt", sep="")
write.table(res, opfn, quote=FALSE, sep="\t", row.names=FALSE, col.names=TRUE)



###########################################
### 2. boxplots of interaction with LDA ###
###########################################

option <- "DiagLDA2"
outdir <- "./DiagLDA2/examples/"
...
###    
   ...
   fn <- paste(option, "/4_lm.results/2_", cell, "_lda", lda, "_trt", treat, "_stratified.FDR.txt", sep="")
   res <- fread(fn, data.table=F, stringsAsFactors=F)

   res2 <- res%>%filter(interaction_ANOVA.qvalue_stratified<0.1)%>%
      mutate(ENSG2=gsub("\\..*", "", ENSG), MCls=cell, LDA=lda, treat=treat)%>%
      left_join(gene_key, by=c("ENSG2"="ensgene"))
    
   res2 <- res2 %>%dplyr::select(ENSG, varID, interaction_ANOVA.pvalue,
      interaction_ANOVA.qvalue, interaction_ANOVA.qvalue_stratified, gene_SNP, ENSG2,
      MCls, LDA, treat, symbol)
   opfn <- paste(outdir, "eGene_SNP.", i, "_", cell, ".lda", lda, ".trt", treat, ".txt", sep="")
   write.table(res2, opfn, row.names=FALSE, quote=FALSE, sep="\t")
} ###



## df <- res2%>%group_by(symbol)%>%summarise(nsnp=n(),.groups="drop")%>%arrange(desc(nsnp))%>%as.data.frame()
## df2 <- df[-1,]
## df2$x <- 1:nrow(df2)
## #sub <- round(seq(1, nrow(df2), length=20))
## sub <- 1:nrow(df2)
## p <- ggplot(df2, aes(x=x, y=nsnp, fill=factor(nsnp)))+
##   geom_bar(stat="identity")+
##   geom_text(data=df2[sub,],
##      aes(x=x, y=nsnp, label=nsnp), size=2, nudge_y=-0.5, color="white")+  
##   ## geom_text_repel(aes(label=symbol), size=2, colour="blue")+
##   ylab("Number of eQTLs")+
##   scale_x_continuous(breaks=sub, labels=df2$symbol[sub])+  
##   coord_flip()+  
##   theme_bw()+  
...
## figfn <- paste(option, "/plots/Hist_", i, ".", cell, ".lda", lda, ".trt", treat, "_NO.eQTLs.png", sep="")
## png(figfn, width=800, height=480, res=130)
## print(p)
## dev.off()


#################################
### boxplot of example eGenes ###
#################################

### boxplot function
boxplot2 <- function(df, varID, symbol, col0, divided=TRUE){
  ...
  if(divided){  
   ...
   ## scale_fill_manual(values= c("0"="#4daf4a", "1"="#984ea3","2"="#ff7f00"))+
       ...
       fig <- ggplot(data=df, aes(x=as.factor(x), y=y))+
          geom_boxplot(fill=col0, outlier.shape=NA)+
...
### create directory
outdir <- paste(option, "/examples/boxplot_LDA-interaction/",
   cell, "_lda", lda, "_trt", treat, "_HLA-DRB5", sep="")
...
###
  ...
  ###  
  fig <- boxplot2(df, varID, symbol, col0)+
  ...
  png(figfn, width=480, height=600, res=120)
  print(fig)
  dev.off()
}
